"""Hospital subsampling experiment to validate saturation phenomenon."""
import sys
...
train_full, val_full, test_full = prepare_monash_dataset('hospital', prediction_length=12)

all_items = train_full.index.levels[0].tolist()
print(f"Total items: {len(all_items)}")

# Subsample sizes (hospital has 767 items)
sizes = [50, 100, 250, 500, len(all_items)]
results = []

for size in sizes:
    print(f"\n{'='*60}")
    print(f"Subsampling {size} items from hospital")
    print(f"{'='*60}")
    ...
    # Zero-shot
    model_dir = f"/tmp/hospital_sub_{size}_zs"
    ...
    # LoRA
    model_dir = f"/tmp/hospital_sub_{size}_lora"
    shutil.rmtree(model_dir, ignore_errors=True)
...
results_df.to_csv('/root/chronos-2-project/results/hospital_subsampling_results.csv', index=False)
print("\n=== Hospital Subsampling Results ===")
...
axes[0].set_title('Hospital: Performance vs. Dataset Scale')
axes[0].legend()
axes[0].grid(True, alpha=0.3)

axes[1].bar(results_df['size'].astype(str), results_df['gain_pct'], color=['#2ecc71']*4 + ['#e74c3c'], width=0.6)
axes[1].axhline(y=0, color='gray', linestyle='--', alpha=0.5)
axes[1].set_xlabel('Number of Series')
axes[1].set_ylabel('LoRA Gain (%)')
axes[1].set_title('Hospital: Fine-tuning Benefit vs. Scale')
axes[1].grid(True, alpha=0.3, axis='y')

plt.tight_layout()
plt.savefig('/root/chronos-2-project/results/hospital_subsampling_curve.png', dpi=200, bbox_inches='tight')
plt.close()
print("\nPlot saved to: /root/chronos-2-project/results/hospital_subsampling_curve.png")
