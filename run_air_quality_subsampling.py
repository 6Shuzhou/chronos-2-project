"""Air quality subsampling experiment to validate negative transfer at different scales."""
import sys
...
# Load full air_quality data
loader = CSVTimeseriesLoader('/root/datasets/air_quality')
train_full, val_full, test_full = loader.load()

all_items = train_full.index.levels[0].tolist()
print(f"Total items: {len(all_items)}")

# Subsample sizes (air_quality has 76 items)
sizes = [50, len(all_items)]
results = []

for size in sizes:
    print(f"\n{'='*60}")
    print(f"Subsampling {size} items from air_quality")
    print(f"{'='*60}")
    ...
    # Zero-shot
    model_dir = f"/tmp/air_quality_sub_{size}_zs"
    ...
    # LoRA-ES
    model_dir = f"/tmp/air_quality_sub_{size}_lora"
    shutil.rmtree(model_dir, ignore_errors=True)
...
results_df.to_csv('/root/chronos-2-project/results/air_quality_subsampling_results.csv', index=False)
print("\n=== Air Quality Subsampling Results ===")
